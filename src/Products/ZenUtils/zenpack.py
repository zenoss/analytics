...
from Products.ZenMessaging.queuemessaging.schema import (
    removeZenPackQueuesExchanges,
)
from Products.ZenModel import ZenPackLoader as ZPL
...
def RemoveZenPack(
    ...
    root = os.path.join(Products.__path__[-1], packName)
    if deleteFiles:
        if log:
        ...
        if os.path.isdir(root):
            recurse = "r"
        os.system("rm -%sf %s" % (recurse, root))  # noqa: S605
    cleanupSkins(dmd)
    return True
...
class ZenPackCmd(ZenScriptBase):
        ...
        while True:
            try:
                ...
                if timeout is None or elapsed > timeout:
                    log.info(
                        "ZEP did not become ready within %s seconds. "
                        "Returning False.",
                        timeout,
                    )
                    return False
    ...
    def run(self):
        ...
        if self.args:
            print(
                "Require one of --install, --remove, --export, --create, "
                "or --list flags."
            )
        ...
        if self.options.installPackName:
            audit(
                "Shell.ZenPack.Install", zenpack=self.options.installPackName
            )
        ...
        from . import ZenPackCmd as EggPackCmd
        ...
        if self.options.createPackName:
            ...
            try:
                self.connect()
                self.dmd.ZenPackManager.manage_addZenPack(
                    packName, devDir=devDir
                )
        ...
        if self.options.installPackName:
            eggInstall = self.options.installPackName.lower().endswith(
                ".egg"
            ) or os.path.exists(
                os.path.join(self.options.installPackName, "setup.py")
            )

        if self.options.removePackName and self.options.filesOnly:
            # A files-only remove implies a files-only install.
            # Egg packs are only supported here.
            installedEggs = list(iter_entry_points("zenoss.zenpacks"))
            theOne = filter(
                lambda x: x.name == self.options.removePackName, installedEggs
            )
            if not theOne:
            ...
            if len(theOne) > 1:
                raise ZenPackException(
                    "Multiple matching distributions for {} found - "
                    "aborting.".format(self.options.removePackName)
                )
            ...
            try:
                os.system(  # noqa: S605
                    'pip uninstall -y "%s"' % self.options.removePackName
                )
            except Exception as ex:
                self.log.error(
                    'Could not uninstall "%s"', self.options.removePackName
                )
        ...
        if (
            self.options.installPackName or self.options.removePackName
            ...
            print(
                "Execute 'zeneventserver start' and retry the "
        ...
        if self.options.installPackName:
            ...
            if eggInstall:
                try:
                    ...
                    if self.options.link:
                        self.stop(
                            "%s cannot be installed with --link option"
                            % self.options.installPackName
                        )
                    else:
                        self.stop(
            ...
            # We want to make sure all zenpacks have a skins directory and
            # that it is registered. The zip file may not contain a skins
            # directory, so we create one here if need be.  The directory
            # should be registered by the zenpack's __init__.py and the skin
            # should be registered by ZPLSkins loader.
            skinsSubdir = os.path.join(
                Products.__path__[-1], packName, "skins", packName
            )
            ...
            try:
                ...
                if not pack:
                    ...
                    RemoveZenPack(
                        self.dmd, self.options.removePackName, self.log
                    )
            finally:
    ...
    def getPacksPacks(self):
        ...
        if os.path.isdir(packsPath):
            return [
                f
                for f in os.listdir(packsPath)
                if os.path.isfile(os.path.join(packsPath, f))
                and f.endswith(".egg")
            ]
        else:
            return []
    ...
    def onlyInImage(self):
        ...
        genericShippedPacks = [
            Distribution.from_filename(pack).project_name
        ...
        packsInImageNotInDB = (
            set(installedPacks) | set(genericShippedPacks)
        ) - set(databasePacks)
    ...
    def restore(self):
        ...
        try:
            with open(UPGRADE_FROM_FILE, "r") as f:
                ...
                log.info(
                    "Installing zenpacks that are new since %s", from_version
                )
        except Exception:
            log.info(
            ...
            if os.path.isfile(ZPHISTORY):
                ...
                for zp in zphistory:
                    ...
                    if zp_version > from_version and zp_version <= to_version:
                        self.log.info(
                            "Zenpack %s (new since %s) to be installed.",
                            zp,
                            zp_version,
                        )
                        zpsToRestore[zp] = (
                            get_distribution(zp).version,
                            False,
                            ZPSource.disk,
                            False,
                        )
                        ...
                        self.log.info(
                            "Zenpack %s (new since %s) is not new.",
                            zp,
                            zp_version,
                        )
        ...
        for zpId in self.dmd.ZenPackManager.packs.objectIds():
            ...
            try:
                ...
                versionCmp = cmp(
                    parse_version(get_distribution(zpId).version),
                    parse_version(version),
                )
                if versionCmp != 0:
                    restoreZenPack = True
                    # The distribution package version is higher than
                    # what's in zodb
                    if versionCmp > 0:
                        version = get_distribution(zpId).version
                        filesOnly = False
                        zpSource = ZPSource.disk
                    # Zodb has a higher version that what's in the
            ...
            if version is None:
                self.log.error(
                    "Could not determine version of %s, skipping.", zpId
                )
                continue
            if restoreZenPack:
        ...
        for zpId, zpDetails in zpsToRestore.items():
            if zpDetails[2] == ZPSource.disk:
                ...
                for dep in zp.requires():
                    deps[dep.unsafe_name] = (
                        ""  # TODO: need to have version as value in dict
                    )
            ...
            matches = filter(
                lambda x: x, [re.search(pattern, key) for key in deps.keys()]
            )
            ...
            if len(deps) != 0 and any(matches):
                for match in matches:
                    ...
                    if depName in zpsToRestore.keys():
                        ...
                        pack.id for pack in self.dmd.ZenPackManager.packs()
                    ]:  # pack not installed in DB
                        depsSet.add(depName)
        ...
        def doRestore(pack):
            try:
                self._restore(
                    pack, zpsToRestore[pack][0], zpsToRestore[pack][1]
                )
                return True
        ...
        # exlude zenpacks which requires files-only install since we've done
        # that already.
        sortedPacks = [
            pack for pack in sortedPacks if not zpsToRestore[pack][1]
        ]
        while len(sortedPacks) > 0:
            packListLen = len(sortedPacks)
            # Keep track of all the packs that failed to restore
            self.log.info(
                "Attempting to install packs: %s", ", ".join(sortedPacks)
            )
            sortedPacks[:] = [
                pack for pack in sortedPacks if not doRestore(pack)
            ]
            if len(sortedPacks) == packListLen:
                if triedReversing:
                    self.log.error(
                        "Unable to install zenpacks: %s",
                        ", ".join(sortedPacks),
                    )
                    sys.exit(1)
                else:
                ...
                self.log.info(
                    "Failed to install packs: %s, will try again",
                    ", ".join(sortedPacks),
                )
        ...
        for pack in self.onlyInImage():
            ...
            with open(os.devnull, "w") as fnull:
                ...
                subprocess.check_call(cmd, stdout=fnull, stderr=fnull)  # noqa: S603
    ...
    def _findEggs(self, zenpackID, zenpackVersion):
        ...
        for dirpath in zenPath(".ZenPacks"), zenPath("packs"):
            for f in os.listdir(dirpath):
                ...
                if name == zenpackID and (
                    not zenpackVersion or version == zenpackVersion
                ):
    ...
    def _restore(self, zenpackID, zenpackVersion, filesOnly):
        ...
        if len(eggs) == 0:
            self.log.info(
                "Could not find install candidate for %s (%s)",
                zenpackID,
                zenpackVersion,
            )
            return
            ...
            eggpaths = ", ".join(eggs)
            self.log.error(
                "Found more than one install candidate for %s (%s): "
                "%s [skipping]",
                zenpackID,
                zenpackVersion,
                eggpaths,
        ...
        if candidate.lower().endswith(".egg"):
            try:
                ...
                cmd.extend(
                    [
                        "--install",
                        os.path.join(
                            tempfile.gettempdir(), os.path.basename(candidate)
                        ),
                    ]
                )
            ...
            self.log.warning(
                "non-egg zenpacks can not currently be restored "
                "automatically: %s",
                candidate,
            )
    ...
    def preInstallCheck(self, eggInstall=True):
        ...
        if eggInstall:
            ...
            if self.options.installPackName.lower().endswith(".egg"):
                ...
                if not os.path.exists(self.options.installPackName):
                    raise ZenPackNotFoundException(
                        "Unable to find ZenPack named '%s'"
                        % self.options.installPackName
                    )
            ...
            for req in reqZenpacks:
                ...
                for parsed_req in parse_requirements([req]):
                    ...
                    if installed_version is None:
                        ...
                        if installed_version not in parsed_req:
                            self.log.error(
        ...
        if parser.has_section(CONFIG_SECTION_ABOUT) and parser.has_option(
            CONFIG_SECTION_ABOUT, "requires"
        ):
            requires = eval(parser.get(CONFIG_SECTION_ABOUT, "requires"))  # noqa: S307
            ...
            if missing:
                self.log.error(
                    "ZenPack %s was not installed because",
                    self.options.installPackName,
    ...
    def extract(self, fname):
        ...
        for name in zf.namelist():
            fullname = os.path.join(Products.__path__[-1], name)
    ...
    def copyDir(self, srcDir):
        ...
        if not os.path.isdir(srcDir):
            self.stop(
                "Specified directory does not appear to exist: %s" % srcDir
            )
        ...
        if os.path.exists(root) and os.path.samefile(root, srcDir):
            self.log.debug(
                "Directory already in %s, not copying.", Products.__path__[-1]
            )
            return packName
    ...
    def linkDir(self, srcDir):
        ...
        targetdir = os.path.join(Products.__path__[-1], packName)
        cmd = "test -d %s && rm -rf %s" % (targetdir, targetdir)
        os.system(cmd)  # noqa: S605
        cmd = "ln -s %s %s" % (srcDir, Products.__path__[-1])
        os.system(cmd)  # noqa: S605
    ...
    def buildOptions(self):
        self.parser.add_option(
            ...
            dest="createPackName",
            default=None,
            help="Zenpack name or path to full destination path, e.g. "
            "/home/zenoss/src/ZenPacks.example.TestPack",
        )
        self.parser.add_option(
            "--install",
        ...
        self.parser.add_option(
            ...
            type="string",
            default=[],
            help="(Can be used multiple times) with --restore, pack name "
            "to not be deleted from disk if not found in zodb",
        )
        self.parser.add_option(
            "--link",
            dest="link",
